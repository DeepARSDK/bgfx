...
#include "glimports.h"
	...
	struct TextureGL
	{
		...
		bool init(GLenum _target, uint32_t _width, uint32_t _height, uint32_t _depth, uint8_t _numMips, uint32_t _flags, bool _genMipmaps = false);
		void create(const Memory* _mem, uint32_t _flags, uint8_t _skip, uint32_t nativeHandle, bool _genMipmaps = false);
